...
class IncomeTest(TestCase):
    ...
    def test_create_income(self):
        ...
        income = Income.objects.filter(amount=1000, account=Account.objects.get(id=1), source="payment",
    ...
    def test_get_income(self):
        ...
        result = json.loads(response.content)['result']
        expect = {'id': 1, 'amount': '1000.00', 'date': timezone.now().strftime("%Y-%m-%d"), 'account': 1, 'source': 'payment',
                  'note': 'For test'}
        self.assertEqual(result, expect)
